...
(function(){
  ...
  this.Dalmatian.template = _.template;

  /**
   * @class Dalmatian.View
   * @description 负责显示模块的抽象层
   */
  this.Dalmatian.View = _.inherit({
    ...
    render: function (status, data, callback) {
      ...
      if (templateSelected) {
        ...
        if (data) {
          // 渲染view
          var templateFn = this.Dalmatian.template(templateSelected);
  ...
  /**
   * @class Dalmatian.Adapter
   * @description 负责承接数据模型的Adapter
   */
  this.Dalmatian.Adapter = _.inherit({
  ...
  /**
   * @class Dalmatian.ViewController
   * @description 负责对View和Adapter的控制
   */
  this.Dalmatian.ViewController = _.inherit({
    ...
    _verify: function (options) {
      ...
      if (!_.property('view')(options) && (!this.view)){
        throw new Error('view必须在实例化的时候传入ViewController');
      }

      if (!_.property('viewstatus')(options) && (!this.viewstatus)){
        throw new Error('ViewController初始化时必须获得viewstatus值');
      }
    },
